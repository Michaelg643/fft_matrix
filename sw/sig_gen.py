...
def determine_bins(raw_freq, npts, min_freq, max_freq):
    ...
    for freq in freq_array:
        ...
        freq_dict["freq_delta"] = np.abs(
            freq_dict["closest_fft_freq"] - freq_dict["freq"]
        )[0]
    ...
    for i in range(array_len):
        ...
        if i < (array_len - 1):
            dist_to_next = (
                freq_dict_array[i + 1]["closest_idx"]
                - freq_dict_array[i]["closest_idx"]
            )
            freq_dict_array[i]["stop_idx"] = int(
                freq_dict_array[i]["closest_idx"] + np.round(dist_to_next / 2)
            )
        else:
            freq_dict_array[i]["stop_idx"] = usable_fft_pts

        freq_dict_array[i]["sum_length"] = int(
            freq_dict_array[i]["stop_idx"] - freq_dict_array[i]["start_idx"] + 1
        )
...
def plot_spectrum(fs, sp, freq, sp_norm, freq_norm):
    ...
    plt.subplot(2, 1, 2)
    plt.bar(
        freq_norm[:-1], sp_norm[:-1], width=np.diff(freq_norm), ec="k", align="edge"
    )
