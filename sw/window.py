...
def approximate_b(A_sl):
    ...
    assert (A_sl > 0) and (
        A_sl <= 120
    ), f"A_sl needs to be between 0 and 120, got {A_sl}"
    if A_sl <= 13.26:
        b = 0
...
def fixed_point_kaiser2(M, beta, nbits):
    win = kaiser2(M, beta)
    kaiser_win = (2**nbits - 1) * win
    return kaiser_win, win
...
def write_out_window(win, fname, nbits, cname):
    ...
    assert (
        max(win) < 2**nbits
    ), "ERROR: Maximum value for window cannot be contained in nbits. Reduce window amplitude or increase bit width"
    ...
    with open(fname, "w") as f_handle:
        print(
            f"type lut is array (natural range 0 to {len(win)-1}) of std_logic_vector({nbits-1} downto 0);",
            file=f_handle,
        )
